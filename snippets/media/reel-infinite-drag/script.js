  ...
  // Design note: an endless horizontal field — drag with inertia, snap to
  // slides, arrows + click/keyboard to index. The counter rolls like a
  // mechanical register and the rail tracks position. User-initiated, so it
  // stays enabled under reduced motion.
  gsap.registerPlugin(Draggable, InertiaPlugin);

  const list = document.querySelector('[data-reel="list"]');
  const viewport = document.querySelector('[data-reel="viewport"]');
  if (list) list.style.flexWrap = "nowrap";
  ...
  const loop = horizontalLoop(slides, {
    paused: true, draggable: true, center: false, trigger: viewport,
    onChange: (el, index) => {
      slides.forEach((s) => s.classList.remove("is-active"));
      el.classList.add("is-active");
      gsap.to(nums, { y: `-${100 * index}%`, ease: "power3.out", duration: 0.5 });
      gsap.to('[data-reel="rail"]', { scaleX: (index + 1) / slides.length, ease: "power3.out", duration: 0.5 });
    }
  });

  document.querySelector('[data-reel="next"]').addEventListener("click", () => loop.next({ ease: "power3.inOut", duration: 0.7 }));
  document.querySelector('[data-reel="prev"]').addEventListener("click", () => loop.previous({ ease: "power3.inOut", duration: 0.7 }));
  slides.forEach((slide, i) => {
    slide.addEventListener("click", () => loop.toIndex(i, { ease: "power3.inOut", duration: 0.7 }));
    slide.addEventListener("keydown", (e) => {
      if (e.key === "Enter" || e.key === " ") { e.preventDefault(); loop.toIndex(i, { ease: "power3.inOut", duration: 0.7 }); }
    });
  });

  /* GSAP community helper — infinite horizontal loop with draggable + inertia */
  function horizontalLoop(items, config) {
    ...
    gsap.context(() => {
      let onChange = config.onChange, lastIndex = 0,
        ...
        spaceBefore = [], xPercents = [], curIndex = 0, indexIsDirty = false,
      ...
      if (config.draggable && typeof (Draggable) === "function") {
        ...
        let wrap = gsap.utils.wrap(0, 1), ratio, startProgress, draggable, dragSnap, lastSnap, initChangeX, wasPlaying,
